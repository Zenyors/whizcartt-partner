...
const StoreContent: React.FC<StoreContentProps> = ({ 
  ...
  const { notifyProductAction } = useNotificationService();
              ...
              <div>
                ...
                <div className="text-sm">
                  {product.discount?.enabled ? (
                    <div className="flex items-center">
                      <span className="text-gray-500 line-through mr-2">₹{parseFloat(product.price).toFixed(2)}</span>
                      <span>
                        ₹{product.discount.type === 'percentage' 
                          ? (parseFloat(product.price) - (parseFloat(product.price) * Number(product.discount.amount) / 100)).toFixed(2)
                          : (parseFloat(product.price) - Number(product.discount.amount)).toFixed(2)
                        }
                      </span>
